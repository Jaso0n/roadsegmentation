...
if( exist('x','var') == 0 && exist('y','var') == 0)
    subplot(2,2,2),imshow(I,[]);title('灰度图（选取种子点）');
    %hold on;
...
reg_min = I(x, y);%表示分割好的区域像素最大值，初始化为种子的灰度值
...
pixdist = 0;%记录最新像素点增加到分割区域后的距离测度,初始化为0
...
while I(x,y) > (dist_ga*1.3/255) && reg_size<numel(I)
